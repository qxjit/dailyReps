...
module Year2020.Month03.Day12 where
...
instance Eq Pies where
  left == right =
    case (left, right) of
      (Cherry, Cherry) -> True
      (Apple, Apple) -> True
      (LemonMeringue, LemonMeringue) -> True
      _ -> False
...
instance Eq Name where
  left == right =
    let
      toLowerString = map Char.toLower . nameToString
    in
      toLowerString left == toLowerString right
